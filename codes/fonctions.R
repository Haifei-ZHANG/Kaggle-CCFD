...
KNN = function(train_under_x,train_under_y,test_under_x,test_under_y,train_over_x,train_over_y,test_over_x,test_over_y){
  ...
  knn.under.roc <<- roc(test_under_y, as.numeric(as.character(knn.under.pred)))
  ...
  knn.over.roc <<- roc(test_over_y, as.numeric(as.character(knn.over.pred)))
  Performances <<- add_Performance("KNN_over",knn.over.pred,test_over_y,AUC=knn.over.roc$auc)
  
  plot(knn.under.roc,main="ROC pour KNN",col = 'blue')
...
SVM = function(train_under_x,train_under_y,test_under_x,test_under_y,train_over_x,train_over_y,test_over_x,test_over_y){
  library(pROC)
  library(e1071)
  #svm.model = svm(bagging_train_x,bagging_train_y,scale = TRUE)
  #svm.pred = predict(svm.model,newdata = test_x)
  #svm.roc = roc(test_data$Class, as.numeric(svm.pred))
  #Performances = add_Performance("SVM",svm.pred,test_data$Class,AUC=svm.roc$auc)
...
#ROC pour l'analyse discriminante
pred <- predict(lda.model,newdata = test_data)
lda.roc <<- roc(test_data$Class, as.numeric(pred$class))
pred <- predict(lda.under.model,newdata = test_data)
lda.under.roc <<- roc(test_data$Class, as.numeric(pred$class))
pred <- predict(lda.over.model,newdata = test_data)
lda.over.roc <<- roc(test_data$Class, as.numeric(pred$class))

pred <- predict(qda.model,newdata = test_data)
qda.roc <<- roc(test_data$Class, as.numeric(pred$class))
pred <- predict(qda.under.model,newdata = test_data)
qda.under.roc <<- roc(test_data$Class, as.numeric(pred$class))
pred <- predict(qda.over.model,newdata = test_data)
qda.over.roc <<- roc(test_data$Class, as.numeric(pred$class))

pred <- predict(nb.model,newdata = test_data)
nb.roc <<- roc(test_data$Class, as.numeric(pred))
pred <- predict(nb.under.model,newdata = test_data)
nb.under.roc <<- roc(test_data$Class, as.numeric(pred))
pred <- predict(nb.over.model,newdata = test_data)
nb.over.roc <<- roc(test_data$Class, as.numeric(pred))

plot(lda.roc,main="ROC pour l'analyse discriminante")
plot.roc(lda.under.roc,add = TRUE ,col = 'blue')
plot.roc(lda.over.roc,add = TRUE ,col = 'red')
plot.roc(qda.roc,add = TRUE ,col = 'deeppink')
plot.roc(qda.under.roc,add = TRUE ,col = 'green')
plot.roc(qda.over.roc,add = TRUE ,col = 'gray6')
plot.roc(nb.roc,add = TRUE ,col = 'yellow')
plot.roc(nb.under.roc,add = TRUE ,col = 'sandybrown')
plot.roc(nb.over.roc,add = TRUE ,col = 'hotpink')

legend("bottomright",inset=.05,c("LDA","LDA_under","LDA_over","QDA","QDA_under","QDA_over","NB","NB_under","NB_over"),
       lty=c(rep(1,9)),col=c("black","blue","red","deeppink","green","gray6","yellow","sandybrown","hotpink"))


#ROC pour SVM
pred <- predict(svm.model,newdata = test_x)
svm.roc <- roc(test_data$Class, as.numeric(pred))
pred <- predict(svm.under.model,newdata = test_x)
svm.under.roc <<- roc(test_data$Class, as.numeric(pred))
pred <- predict(svm.over.model,newdata = test_x)
svm.over.roc <<- roc(test_data$Class, as.numeric(pred))

plot(svm.roc,main="ROC pour SVM")
plot.roc(svm.under.roc,add = TRUE ,col = 'blue')
plot.roc(svm.over.roc,add = TRUE ,col = 'red')
legend("bottomright",inset=.05,c("SVM","SVM_under","SVM_over"),lty=c(1,1,1),col=c("black","blue","red"))

lda.model <<- lda(Class~.,data = train_data)
lda.pred <<- predict(lda.model, newdata = test_data)
Performances <<- add_Performance("LDA0",lda.pred$class,test_data$Class)

lda.model1 <<- lda(Class~V1+V2+V3+V4+V5+V6+V7+V8+V9+V10+V11+V12+V13+V14+V15+V16+V17+V18+V19+Amount,data = train_data)
lda.pred <<- predict(lda.model1, newdata = test_data)
Performances <<- add_Performance("LDA1",lda.pred$class,test_data$Class)

lda.model2 <<- lda(Class~V1+V2+V3+V4+V5+V6+V7+V8+V9+V10+V11+V12+V14+V16+V17+V18+V19+V21+V22+V24+V25+V26+V27+V28+Amount,data = train_data)
lda.pred <<- predict(lda.model2, newdata = test_data)
Performances <<- add_Performance("LDA2",lda.pred$class,test_data$Class)
